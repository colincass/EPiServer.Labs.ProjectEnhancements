Param(
    $SiteName = "project-enhancements-test",
    # $SitePath = "$SiteRoot\$SiteName",
    $PackageName = "Alloy.Sample.ProjectEnhancements",
...
$SiteRoot = "e:\ProjectEnhancements"
$IISSiteName = "Default Web Site\ProjectEnhancements"
...
function Transform-Config {
    ...
    Update-EPiXmlFile -TargetFilePath "$SitePath\wwwroot\Web.config" -ModificationFilePath "$TmpPackageFolder\Setup\Alloy.Sample.ProjectEnhancements.web.config.transform" -Replaces "{basePath}=$SitePath\appData;{DbServer}=$DbServer;{DbDatabase}=$SiteName;{DbUserName}=$DbSiteUser;{DbPassword}=$DbSitePassword;"
}
